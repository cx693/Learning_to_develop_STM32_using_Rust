...
# 注意如果图片显示失败
下载本项目使用md阅读器打开！（VsCode可以）

# 目录

- [声明](#声明)
- [基础环境搭建](#基础环境搭建)
  - [安装probe-rs](#安装probe-rs)
  - [安装编译器](#安装编译器)
  - [使用probe-rs检测](#使用probe-rs检测)
  - [安装软件包](#安装软件包)
- [编译测试](#编译测试)
  - [函数无法跳转](#函数无法跳转)
- [调试](#调试)
- [项目下载（固化）](#项目下载固化)
  - [HEX文件](#hex文件)
  - [BIN文件](#bin文件)
- [解除JTAG端口-无法下载](#解除jtag端口-无法下载)
- [学习地址](#学习地址)
- [基础学习](#基础学习)
  - [时钟系统详解](#时钟系统详解)
    - [STM32F1 时钟树总览](#stm32f1-时钟树总览)
    - [时钟源详解](#时钟源详解)
    - [各总线时钟详解](#各总线时钟详解)
    - [stm32f1xx-hal 中的时钟配置](#stm32f1xx-hal-中的时钟配置)
    - [常用时钟配置方案](#常用时钟配置方案)
    - [时钟配置常见错误](#时钟配置常见错误)
    - [时钟配置速查表](#时钟配置速查表)
  - [点亮第一颗LED](#点亮第一颗led)
  ...
  - [LED闪烁](#led闪烁)
    - [SYST 模式延时闪烁](#syst-模式延时闪烁)
    - [LED闪烁-TIM2定时器延时](#led闪烁-tim2定时器延时)
    - [延时讲解](#延时讲解)
  - [按键点灯](#按键点灯)
  - [动态GPIO切换（复用端口）](#动态gpio切换复用端口)
  - [外部中断EXTI](#外部中断exti)
    - [EXTI 中断线与引脚对应关系](#exti-中断线与引脚对应关系)
    - [关键步骤总结](#关键步骤总结)
  - [定时器中断](#定时器中断)
    - [定时器中断闪烁LED](#定时器中断闪烁led)
    - [RTIC定时器中断模式](#rtic定时器中断模式)
  - [RTIC2异步任务](#rtic2异步任务)
  - [串口通信](#串口通信)
    - [普通模式](#普通模式)
    - [串口通信_fmt 模式](#串口通信_fmt-模式)
    - [串口中断_空闲检测](#串口中断_空闲检测)
    - [串口DMA接收](#串口dma接收)
  - [ADC采集](#adc采集)
    - [采集外部电压](#采集外部电压)
    - [内部ADC采集转换温度](#内部adc采集转换温度)
    - [ADC_DMA_循环采集](#adc_dma_循环采集)
  - [SPI协议](#spi协议)
    - [点亮ST7789屏幕](#点亮st7789屏幕)
  - [I2C通信](#i2c通信)
  - [PWM波](#pwm波)
    - [输出](#输出)
    - [输入](#输入)
...
- [实际项目](#实际项目)
  - [DHT11](#dht11)
  - [DHT11+ST7789液晶屏温度计](#dht11st7789液晶屏温度计)
  - [DHT20+ST7789液晶屏温度计](#dht20st7789液晶屏温度计)
...
| 2 | **连接到 EXTI 线** | <code>make_interrupt_source(&mut syscfg)</code> | 将引脚连接到对应的 EXTI 中断线 |
| 3 | **设置触发边沿** | <code>trigger_on_edge(&mut exti, Edge::RISING)</code> | 可选择上升沿、下降沿或双沿触发 |
| 4 | **使能 EXTI 中断** | <code>enable_interrupt(&mut exti)</code> | 在 EXTI 外设中启用该中断线 |
...
> 更新: 2026-05-30 18:41:48  
> 原文: <https://www.yuque.com/xianwei-37i3i/xxssga/oibnx39qnlvokmhg>