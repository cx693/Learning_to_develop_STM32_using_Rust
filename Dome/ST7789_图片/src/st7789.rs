...
impl<DC: embedded_hal::digital::OutputPin, RST: embedded_hal::digital::OutputPin> ST7789<DC, RST> {
    ...
    pub fn draw_picture<PULL>(&mut self, spi: &mut Spi<pac::SPI1, u8, PULL>, x: u16, y: u16, data: &[u8], width: u16, height: u16) -> Result<(), SpiError> {
        ...
        while offset < total {
            let end = if offset + row_bytes <= total { offset + row_bytes } else { total };
