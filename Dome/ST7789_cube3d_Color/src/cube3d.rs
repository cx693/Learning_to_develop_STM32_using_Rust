...
impl Cube3D {
    ...
    pub fn project_wireframe(&mut self) {
        let rcx = cos_i(self.angle_x);
        let rsx = sin_i(self.angle_x);
        let rcy = cos_i(self.angle_y);
        let rsy = sin_i(self.angle_y);
    ...
    pub fn take_dirty_rect(&mut self) -> Option<DirtyRect> {
        let (cx, cy, cw, ch) = self.get_bounding_box();
        let cur = DirtyRect { x: cx, y: cy, w: cw, h: ch };
        let merged = match self.prev_bbox.take() {
            Some(prev) => {
                let x0 = if prev.x < cur.x { prev.x } else { cur.x };
                let y0 = if prev.y < cur.y { prev.y } else { cur.y };
                let x1a = prev.x + prev.w;
                let x1b = cur.x + cur.w;
                let y1a = prev.y + prev.h;
                let y1b = cur.y + cur.h;
